...
kubectl wait --for=condition=ready pod -l app=fe -n fith --timeout=60s
kubectl wait --for=condition=ready pod -l app=be -n fith --timeout=60s

echo "=== Testing Network Policies ==="

# Get pod names
FE_POD=$(kubectl get pods -n fith -l app=fe -o jsonpath='{.items[0].metadata.name}')
BE_POD=$(kubectl get pods -n fith -l app=be -o jsonpath='{.items[0].metadata.name}')
...
if kubectl exec -n fith $FE_POD -- curl -s --connect-timeout 5 -o /dev/null -w "%{http_code}" be-service.fith.svc.cluster.local:80 | grep -q "200"; then
...
if timeout 10s kubectl exec -n fith $BE_POD -- curl -s --connect-timeout 5 -o /dev/null -w "%{http_code}" fe-service.fith.svc.cluster.local:80 | grep -q "200"; then
